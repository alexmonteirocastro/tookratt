...
| `POST /chat` | Multi-turn RAG chat: retrieve jobs from Qdrant, then generate a grounded answer via the `Generator` interface (Gemini 2.5 Flash by default; optional local Ollama or instant stub — see [ADR-0007](docs/adr/0007-local-generation-fallback-ollama-qwen3.md) and [CONTRIBUTING.md](CONTRIBUTING.md#local-generation-for-development)). **Request:** `question` (required, max **500** characters — bounds token cost and latency before any retrieval or generation), optional `limit` (default 5), optional `country` (`DK`, `SE`, `NO`, `FI`, `IS`, `EU`), optional `remote` (`true` = remote only, `false` = on-site only), optional `session_id` (omit to start a fresh session). When `country`/`remote` are omitted, `/chat` infers them from the question text via deterministic keyword matching; a follow-up that resolves no signal inherits the session's last-applied filter; explicit request fields always override both. Retrieval embeds only the current question — prior turns are generation context, not query text ([ADR-0008](docs/adr/0008-multi-turn-conversation-memory.md)). **Rate limit:** per-client in-memory limit on `/chat` only (default **10 requests/minute**; returns 429 when exceeded). `/jobs/search` and `/jobs/stats` are not rate-limited. Override via `CHAT_QUESTION_MAX_LENGTH`, `CHAT_RATE_LIMIT`, `CHAT_SESSION_TTL_SECONDS` (default **1800**), `CHAT_HISTORY_MAX_TURNS` (default **5**), and `CHAT_MAX_SESSIONS` (default **1000**) in `.env`. **Response:** `question`, `answer`, `sources` (fused RRF top-k hits with usable `document_text`; dense cosine is display-only — [ADR-0018](docs/adr/0018-chat-sourcing-follows-rrf-rank.md)), `generated` (`true` when the LLM produced the answer), `applied_country` / `applied_remote` (the filters actually used for retrieval, including carried-forward values — `null` when nothing was resolved), `session_id` (always populated; send it on the next turn to continue). See [ADR-0001](docs/adr/0001-llm-provider-strategy.md), [ADR-0002](docs/adr/0002-retrieval-filtering-strategy.md), [ADR-0006](docs/adr/0006-chat-endpoint-hardening.md), [ADR-0007](docs/adr/0007-local-generation-fallback-ollama-qwen3.md), [ADR-0008](docs/adr/0008-multi-turn-conversation-memory.md), [ADR-0009](docs/adr/0009-grounded-inline-job-hyperlinks.md), [ADR-0014](docs/adr/0014-embedding-model-migration.md), and [ADR-0018](docs/adr/0018-chat-sourcing-follows-rrf-rank.md). |
...
- [docs/adr/](docs/adr/) — architectural decision records
  ...
  - [ADR-0008](docs/adr/0008-multi-turn-conversation-memory.md) — multi-turn conversation memory for `/chat` (ALE-184 backend; ALE-185 frontend)
