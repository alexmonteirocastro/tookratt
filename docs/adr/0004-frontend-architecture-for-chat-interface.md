...
- **Addressed (ALE-193):** a second React view (`/stats`) ships with `react-router-dom` (`/chat` + `/stats`). Next.js remains rejected (no SSR). Decision 6's "stats only via Swagger" is superseded for the demo UI — Streamlit is still not reintroduced.
...
- **Routing (ALE-193):** `react-router-dom` `BrowserRouter` in `main.tsx` owns `/chat` and `/stats`. `/` redirects to `/chat`. Cloudflare Pages `public/_redirects` and Docker nginx `try_files` serve `index.html` for those paths. `GET /jobs/stats` is unchanged; the React view is the replacement for the retired Streamlit Jobs tab, not a new backend.
