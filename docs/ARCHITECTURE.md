...
| `OLLAMA_API_KEY` | Optional Bearer token for Ollama Cloud (`https://ollama.com`). Unused for local Ollama; already set in local `.env` for this project. A missing Cloud key surfaces as an upstream 401. | *(set in `.env`)* |
