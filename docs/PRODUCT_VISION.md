...
| **4. Personalized matching + advice** | "5 years Python, what's interesting in Finland?" | A candidate profile (experience, stack, preferences) held across turns and matched against many listings | Bounded conversation memory shipped ([ADR-0008](adr/0008-multi-turn-conversation-memory.md), ALE-184 / ALE-185). The candidate profile itself is still Phase 2. |
...
* Persistent / cross-session conversation history (current memory is in-tab and bounded; see [ADR-0008](adr/0008-multi-turn-conversation-memory.md))
